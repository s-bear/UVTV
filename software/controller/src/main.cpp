...
namespace System { //use a namespace to aid code browsing
...
scpi_result_t Display::Maxc(scpi_t *ctx) {
  uint32_t r, g, b, v;
  if(!SCPI_ParamUInt32(ctx, &r, true)) return SCPI_RES_ERR;
  if(!SCPI_ParamUInt32(ctx, &g, true)) return SCPI_RES_ERR;
  if(!SCPI_ParamUInt32(ctx, &b, true)) return SCPI_RES_ERR;
...
scpi_result_t Display::DotcAll(scpi_t *ctx)
{
  ...
  if(!SCPI_ParamArbitraryBlock(ctx, &value, &len, true))
    return SCPI_RES_ERR;
  if(len != NUM_LEDS) { //1 byte per LED
    SCPI_ErrorPush(ctx, SCPI_ERROR_ILLEGAL_PARAMETER_VALUE);
    return SCPI_RES_ERR;
  }
...
scpi_result_t Display::Mode(scpi_t *ctx) {
  ...
  if(SCPI_ParamIsNumber(&param, false)) {
    ...
    if (!SCPI_ParamIsValid(&param)) return SCPI_RES_ERR; //parser error
  } else {
    SCPI_ErrorPush(ctx, SCPI_ERROR_DATA_TYPE_ERROR);
    return SCPI_RES_ERR;
  }
...
void loop()
{
  ...
  if(read_len > 0) {
    if (echo) SER.write(ser_buffer, read_len);
